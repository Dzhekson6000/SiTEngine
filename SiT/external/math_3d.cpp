...
{
    const float SinHalfAngle = sinf((float)ToRadian(Angle/2));
    const float CosHalfAngle = cosf((float)ToRadian(Angle/2));
...
void Matrix4f::InitPersProjTransform(float FOV, float Width, float Height, float zNear, float zFar)
{
    ...
    const float tanHalfFOV = tanf((float)ToRadian(FOV / 2.0f));
